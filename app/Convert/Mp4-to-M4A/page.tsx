  ...
  const transcode = async () => {
    ...
    if (inputFileRef.current?.files?.length) {
      ...
      reader.onload = async function(e) {
        ...
        await ffmpeg.exec(['-i', 'input.mp4', 'output.m4a'])
        const outputData = (await ffmpeg.readFile('output.m4a')) as any
        if (videoRef.current)
          videoRef.current.src = URL.createObjectURL(new Blob([outputData.buffer], { type: 'audio/mp4' }))
      }
